...
- `updated_by`: Username of the last updater.
- `creation_timestamp`: Creation time in milliseconds since epoch.
- `update_timestamp`: Last update time in milliseconds since epoch.
"""
...
- `model_configs`: Array of model configurations.
...
struct GatewayEndpoint
    endpoint_id::String
    name::String
    model_configs::Array{GatewayEndpointModelConfig}
...
function GatewayEndpoint(data::AbstractDict)
    GatewayEndpoint(
        get(data, "endpoint_id", ""),
        get(data, "name", ""),
        get(data, "model_configs", []) .|> GatewayEndpointModelConfig,
        get(data, "routing_strategy", ""),
        haskey(data, "fallback_config") ? FallbackConfig(data["fallback_config"]) : FallbackConfig("", 0),
        get(data, "created_by", ""),
        get(data, "updated_by", ""),
        get(data, "creation_timestamp", 0),
        get(data, "update_timestamp", 0),
